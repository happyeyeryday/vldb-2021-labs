...
func (e *InsertExec) Next(ctx context.Context, req *chunk.Chunk) error {
	...
	if len(e.children) > 0 && e.children[0] != nil {
		...
		// YOUR CODE HERE (lab4)
		//panic("YOUR CODE HERE")
		err = insertRowsFromSelect(ctx, e)//根据 Select 的 Insert 会使用 `insertRowsFromSelect` 函数进行处理
		return err
	}
	// Hint: step II.3.1
	// YOUR CODE HERE (lab4)
	//panic("YOUR CODE HERE")
	err = insertRows(ctx, e)//普通的insert
	return err
}
...
func (e *InsertExec) Open(ctx context.Context) error {
	if e.SelectExec != nil {
		...
		// YOUR CODE HERE (lab4)
		//panic("YOUR CODE HERE")
		err = e.SelectExec.Open(ctx)//通过 `SelectionExec.Open` 初始化 `SelectionExec`
		return err
	}
